...
def test_imports():
    ...
    try:
        ...
        print("[OK] Events module imported successfully")
        
        from discovery.device_registry import DeviceRegistry, SUTDevice, SUTStatus
        print("[OK] Device registry module imported successfully")
        
        # Test configuration
        config = ConfigManager.load_config()
        print(f"[OK] Configuration loaded: host={config.host}, port={config.port}")
        
        # Test device registry
        registry = DeviceRegistry()
        print("[OK] Device registry created successfully")
        
        # Test event system
        event_bus = EventBus()
        print("[OK] Event bus created successfully")
...
def test_device_registry():
    ...
    print("\n[DEVICE] Testing device registry...")
    ...
    try:
        ...
        print(f"[OK] Device registered: {device.unique_id} at {device.ip}")
        
        # Test device retrieval
        retrieved = registry.get_device_by_id("test_sut_001")
        if retrieved and retrieved.unique_id == "test_sut_001":
            print("[OK] Device retrieval by ID works")
        else:
            print("[FAIL] Device retrieval failed")
            return False
        ...
        if retrieved_by_ip and retrieved_by_ip.unique_id == "test_sut_001":
            print("[OK] Device retrieval by IP works")
        else:
            print("[FAIL] Device retrieval by IP failed")
            return False
        
        # Test device stats
        stats = registry.get_device_stats()
        print(f"[OK] Device stats: {stats}")
        
        return True
        
    except Exception as e:
        print(f"[FAIL] Device registry test failed: {e}")
        return False

def test_events():
    """Test event system"""
    print("\n[EVENT] Testing event system...")
    ...
    try:
        ...
        def test_handler(event):
            ...
            print(f"  [MSG] Received event: {event.event_type.value}")
        ...
        if len(received_events) == 1:
            print("[OK] Event system working correctly")
            return True
        else:
            print("[FAIL] Event system failed")
            return False
            
    except Exception as e:
        print(f"[FAIL] Event system test failed: {e}")
        return False

def test_discovery_logic():
    """Test discovery service logic without network calls"""
    print("\n[SEARCH] Testing discovery logic...")
    ...
    try:
        ...
        print(f"[OK] Registered {len(devices)} devices")
        
        # Test getting online devices
        online_devices = registry.get_online_devices()
        print(f"[OK] Online devices: {len(online_devices)}")
        ...
        if busy_device.current_task == "Running automation":
            print("[OK] Device status change works")
        else:
            print("[FAIL] Device status change failed")
            return False
        
        return True
        
    except Exception as e:
        print(f"[FAIL] Discovery logic test failed: {e}")
        return False

def main():
    """Run simple tests"""
    print("[START] Starting Simple Backend Test Suite")
    print("=" * 50)
    ...
    print("\n" + "=" * 50)
    print(f"[NEXT] Test Results: {success_count}/{total_tests} tests passed")
